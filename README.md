...
- **语音转录 (自适应双引擎)**：自适应支持 `faster-whisper`（CTranslate2 格式模型）与原生 OpenAI Whisper（PyTorch `.pt` 格式，如 `medium.pt`）。首创纯物理路径模型加载策略，100% 绕过网络检测实现完全离线秒级加载，支持 CPU (int8) / GPU (float16) 硬件自适应检测与完美离线资产复用；
- **显存一键安全释放 (Unload GPU)**：首创前台一键“释放显存”设计，通过触发后台垃圾回收及 `torch.cuda.empty_cache()`，将 Whisper 占用的 GPU 显存迅速归还系统，优雅解决多任务显存紧张问题；
- **机器翻译**：集成 `deep-translator` (Google 翻译)，内置 None 守卫、空文本行过滤与单句异常隔离机制；
